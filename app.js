...
function engToMa(v,s){const span=s.max-s.min||1;return s.currentMin+(v-s.min)*(s.currentMax-s.currentMin)/span;}
function maToEng(ma,s){const span=s.currentMax-s.currentMin||1;return s.min+(ma-s.currentMin)*(s.max-s.min)/span;}
...
function renderMain(){
  ...
  $('manualSlider').value=clamp(state.outputMa,4,20);$('manualCurrent').value=fmt(clamp(state.outputMa,0,24),1);
...
function simulateCommand(line){
  ...
  if(line.startsWith('SET:CURRENT:')){const v=Number(line.split(':')[2]);if(Number.isFinite(v))state.outputMa=v;return 'OK';}
...
async function connectSerial(){
  ...
  if(!('serial'in navigator)){alert('Web Serial no está disponible en este navegador. Usá modo Simulación o Chrome compatible.');return;}
  try{state.port=await navigator.serial.requestPort();await state.port.open({baudRate:Number($('baudRate').value)});state.writer=state.port.writable.getWriter();state.reader=state.port.readable.getReader();state.connected=true;$('connectBtn').disabled=true;$('disconnectBtn').disabled=false;$('supportPill').textContent='HC-05 conectado';$('supportPill').className='pill ok';setPill('En línea',true);readLoop();}catch(e){log(e.message,'ERR');}
}
...
async function disconnectSerial(){stopSimulation();try{if(state.reader){await state.reader.cancel();state.reader.releaseLock();}if(state.writer){state.writer.releaseLock();}if(state.port){await state.port.close();}}catch(e){}state.reader=state.writer=state.port=null;state.connected=false;$('connectBtn').disabled=false;$('disconnectBtn').disabled=true;$('supportPill').textContent='Web Serial';$('supportPill').className='pill';setPill('Sin datos');}
...
function startSimulation(){stopSimulation();state.simulation=true;state.connected=true;$('transportMode').value='simulation';$('connectBtn').disabled=true;$('disconnectBtn').disabled=false;$('supportPill').textContent='Simulación activa';$('supportPill').className='pill ok';setPill('Simulando',true);$('modeStatus').textContent='Simulación activa: la entrada sigue la salida con el desvío configurado.';const tick=()=>{state.simPhase+=0.35;const noise=Number($('simNoise').value)||0;let ma=state.outputOpen?0:state.outputMa+Math.sin(state.simPhase)*noise;addInput(ma);renderMain();};tick();state.simTimer=setInterval(tick,Math.max(100,Number($('simInterval').value)||500));}
function stopSimulation(){if(state.simTimer)clearInterval(state.simTimer);state.simTimer=null;state.simulation=false;}
async function applyOutput(ma){if(!$('outputEnabled').checked)return;ma=Number(ma);if(!Number.isFinite(ma))return;state.outputMa=clamp(ma,0,24);state.outputOpen=false;renderMain();await send(`SET:CURRENT:${state.outputMa.toFixed(3)}`);}
...
function scaleFromInputs(prefix){const type=$(prefix+'SensorType').value;const base=TYPES[type]||TYPES.custom;return{type,name:base.name,unit:$(prefix+'SensorUnit').value||base.unit,min:Number($(prefix+'SensorMin').value),max:Number($(prefix+'SensorMax').value),currentMin:Number($(prefix+'CurrentMin').value),currentMax:Number($(prefix+'CurrentMax').value),measuredLow:Number($(prefix+'MeasuredLow').value),measuredHigh:Number($(prefix+'MeasuredHigh').value)};}
...
function stopRamp(){if(state.rampTimer)clearInterval(state.rampTimer);state.rampTimer=null;state.rampState=null;state.rampPaused=false;$('rampStatus').textContent='Rampa detenida.';}
function runRamp(){stopRamp();const r=getRampConfig(),seq=buildRamp(r);if(!seq.length)return;state.rampState={r,seq,i:0,rep:0};$('rampStatus').textContent='Rampa ejecutándose…';state.rampTimer=setInterval(async()=>{if(state.rampPaused)return;const rs=state.rampState;if(!rs)return;if(rs.i>=rs.seq.length){rs.i=0;rs.rep++;if(rs.rep>=r.repeats){stopRamp();$('rampStatus').textContent='Rampa finalizada.';return;}}const eng=rs.seq[rs.i++];await applyOutput(engToMa(eng,state.outScale));},Math.max(100,r.tick));}
...
function bindTabs(){document.querySelectorAll('.tab').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.panel').forEach(x=>x.classList.remove('active'));b.classList.add('active');$('tab-'+b.dataset.tab).classList.add('active');if(b.dataset.tab==='profiles')renderProfiles();if(b.dataset.tab==='chart')drawChart();}));}
function bind(){
  bindTabs();$('connectBtn').onclick=connectSerial;$('disconnectBtn').onclick=disconnectSerial;$('startSimulationBtn').onclick=startSimulation;$('transportMode').onchange=()=>{$('modeStatus').textContent=$('transportMode').value==='simulation'?'Modo simulación listo para usar sin hardware.':'Web Serial seleccionado. Emparejá el HC-05 en Android/PC y luego presioná Conectar.';};
  $('manualSlider').oninput=e=>{const v=Number(e.target.value);$('manualCurrent').value=v.toFixed(1);state.outputMa=v;renderMain();};$('manualSlider').onchange=e=>applyOutput(Number(e.target.value));$('manualCurrent').onchange=e=>applyOutput(Math.round(Number(e.target.value)*10)/10);$('minusBtn').onclick=()=>applyOutput(Math.round((state.outputMa-.1)*10)/10);$('plusBtn').onclick=()=>applyOutput(Math.round((state.outputMa+.1)*10)/10);$('applyCurrentBtn').onclick=()=>applyOutput(Math.round(Number($('manualCurrent').value)*10)/10);
  $('fault36').onclick=()=>applyOutput(3.6);$('fault21').onclick=()=>applyOutput(21);$('faultOpen').onclick=async()=>{state.outputOpen=true;await send('SET:OUTPUT:OPEN');};$('outputEnabled').onchange=async e=>send(e.target.checked?'SET:OUTPUT:ON':'SET:OUTPUT:OFF');
  $('resetStatsBtn').onclick=()=>{state.samples=[];renderMain();};$('runRampBtn').onclick=runRamp;$('pauseRampBtn').onclick=()=>{state.rampPaused=!state.rampPaused;$('pauseRampBtn').textContent=state.rampPaused?'Continuar':'Pausar';};$('stopRampBtn').onclick=stopRamp;
  ...
  $('getStatusBtn').onclick=()=>send('GET:STATUS');$('uploadProfileBtn').onclick=()=>uploadProfile({scale:clone(state.outScale),ramp:getRampConfig()},Number($('profileNumber').value)||1);$('runDeviceProfileBtn').onclick=()=>send(`PROFILE:RUN:${Number($('profileNumber').value)||1}`);$('terminalSendBtn').onclick=()=>{const v=$('terminalInput').value.trim();if(v){send(v);$('terminalInput').value='';}};$('clearTerminalBtn').onclick=()=>{$('terminal').textContent='';};$('clearChartBtn').onclick=()=>{state.chart=[];drawChart();};$('exportCsvBtn').onclick=exportCsv;
